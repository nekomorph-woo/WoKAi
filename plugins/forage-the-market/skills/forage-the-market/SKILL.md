...
description: 消化需求描述并设计版本化路线图。处理从零散想法到完整 PRD 的各种输入规模，通过发散和侦察双轨探索，收敛为精选 feature 列表和分阶段 roadmap。Use when 用户缺乏功能想法、需要灵感、有大段需求/PRD 需要消化、或提到 "forage" / "觅食" / "roadmap" / "功能规划" / "路线图" / "forage-the-market"。
pipeline:
...
- **pick-the-finding** 仅在 Phase 1 首次执行（探索代码库现状）
